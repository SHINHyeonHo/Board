	...
	<form name="searchFrm">
		<c:if test="${not empty deptIdList}">
			<c:forEach var="deptId" items="${deptIdList}" varStatus="status">
			  <label for="${status.index}"><c:if test="${deptId == -9999}">부서없음</c:if><c:if test="${deptId != -9999}">${deptId}</c:if></label>&nbsp;
			  <input type="checkbox" id="${status.index}" name="deptId" value="${deptId}" />&nbsp;&nbsp;  	
			</c:forEach>
		</c:if>	
		<input type="hidden" name="sDeptIdes" />
		<br/>
		<select name="gender" style="height: 25px; width: 60px; margin: 10px 20px 0 0;">
			<option>성별</option>
			<option>남</option>
			<option>여</option>
		</select>
